...
### M3-R3-C Round-003 frozen selection-gate coverage

M3-R3-C is limited to the mechanical application of the already frozen
Round-003 machine gate to the committed evidence artifact. It must not fetch
market data, rerun the historical loader or backtest, recalculate settlement
or funding, tune candidates, or change any performance result. The authoritative
selection command is deferred until the Commit-A source freeze has passed CI:

```text
npm run research:m3r3:select
```

The dedicated synthetic suite covers the exact 11 gate identities, all nine
candidate IDs and frozen order, SHA-256/provenance/schema checks, CONTROL and
candidate fold diagnostics, aggregate-improvement semantics, improved-fold
counts, catastrophic-fold rejection, expectancy/PF/concentration/fee-burden
thresholds, sample-size requirements, tie-breaking, zero-eligible handling,
machine-readable serialization, markdown rendering, and fail-closed invalid
evidence cases. Synthetic fixtures are test-only; no authoritative result is
generated by `npm test`. The baseline-002 freeze decision, if any, is produced
only by the one-time offline command after Commit-A CI and remains separate
from baseline-001 and the frozen backtest policies.

### M3-R3-C.1 recovery source-freeze coverage

The recovery suite adds synthetic integrity tests for the two reviewed defects:
top-level CONTROL whole-universe counts may differ from F1–F6 aggregate
validation counts, and formal/executed identity hashes are field-bound values
that do not require global uniqueness. Same-variant formal/executed equality
and cross-candidate equality are accepted; missing or malformed SHA-256 values
remain fail-closed. Existing tests continue to pin aggregate-validation gate
scope, all 11 gate identities, thresholds, candidate order, complexity tuples,
