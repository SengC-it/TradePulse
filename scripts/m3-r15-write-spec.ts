...
function writeJson(root: string, relativePath: string, expected: unknown, replaceExisting = false): void {
  const target = path.join(root, relativePath);
  const expectedCanonical = stableStringify(expected);
  if (existsSync(target)) {
    const current = JSON.parse(readFileSync(target, "utf8")) as unknown;
    if (stableStringify(current) !== expectedCanonical && !replaceExisting) throw new Error(`R15 committed specification differs: ${target}`);
...
writeJson(root, R15_CONFORMANCE_PATH, R15_CONFORMANCE_DOCUMENT, true);
validateR15Plan();
