...
type FrozenGateId = Exclude<keyof typeof BASELINE_002_RESEARCH_ROUND_003_MACHINE_RECORD.selectionGates, "researchRoundId" | "sourceSha" | "complexityTieThreshold" | "simplerCandidateRule">;
...
function frozenGateSignatures(): readonly (readonly [string, number, string])[] {
  return BASELINE_002_RESEARCH_ROUND_003_DEFINITIONS.hardGateIdentities.map((gateId) => {
    const gate = BASELINE_002_RESEARCH_ROUND_003_MACHINE_RECORD.selectionGates[
      gateId as FrozenGateId
    ];
    return [gateId, gate.value, gate.comparison] as const;
  });
}
...
describe("M3-R3-C Round-003 mechanical selection", () => {
  const cases: readonly [string, () => void][] = [
    ...
    ["73 top-level CONTROL counts may differ from validation counts", () => { const e = makeEvidence(); e.controlExecutedTrades = 7_495; e.controlFormalSignals = 7_500; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).integrityStatus).toBe("COMPLETE"); }],
    ["74 same-variant formal and executed hashes are accepted", () => { const e = makeEvidence(); const value = candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[0]!); value.executedIdentitySha256 = value.formalIdentitySha256; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).integrityStatus).toBe("COMPLETE"); }],
    ["75 cross-candidate identical formal hashes are accepted", () => { const e = makeEvidence(); const first = candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[0]!); const second = candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[1]!); second.formalIdentitySha256 = first.formalIdentitySha256; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).integrityStatus).toBe("COMPLETE"); }],
    ["76 cross-candidate identical executed hashes are accepted", () => { const e = makeEvidence(); const first = candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[0]!); const second = candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[1]!); second.executedIdentitySha256 = first.executedIdentitySha256; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).integrityStatus).toBe("COMPLETE"); }],
    ["77 missing identity hash fails closed", () => { const e = makeEvidence(); delete candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[0]!).formalIdentitySha256; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).finalDecision).toBe("INCOMPLETE_EVIDENCE"); }],
    ["78 malformed identity hash fails closed", () => { const e = makeEvidence(); candidate(e, M3_R3_ROUND_003_CANDIDATE_IDS[0]!).executedIdentitySha256 = "not-a-sha256"; expect(evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256).finalDecision).toBe("INCOMPLETE_EVIDENCE"); }],
    ["79 recovery fixes preserve frozen gate identities, thresholds, order, and complexity", () => { const e = makeEvidence(); const report = evaluateM3R3CSelection(e, M3_R3_C_EXPECTED_INPUT_EVIDENCE_SHA256); expect(report.candidates.map((value) => value.candidateId)).toEqual([...M3_R3_ROUND_003_CANDIDATE_IDS]); expect(report.candidates.map((value) => value.complexity)).toEqual(M3_R3_ROUND_003_CANDIDATE_IDS.map(complexity)); expect(report.candidates[0]!.gateResults.map((value) => [value.gateId, value.threshold, value.comparison])).toEqual(frozenGateSignatures()); }],
