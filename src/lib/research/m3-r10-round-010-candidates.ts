...
import { R10_FEATURE_NAMES, M3_R10_CANDIDATE_IDS } from "./m3-r10-round-010-protocol.ts";
import { featureVectorFromOrderedValues, type R10FeatureVector } from "./m3-r10-round-010-model.ts";
import { buildR10E1RiskGeometry, buildR10E2RiskGeometry } from "./m3-r10-round-010-risk-geometry.ts";

type NumericSeries = readonly (number | null)[];

export type R10SymbolIndicatorContext = Readonly<{
...
export type R10FeatureContext = Readonly<{
  bySymbol: Readonly<Record<ResearchSymbol, R10SymbolIndicatorContext>>;
}>;

export type R10OpportunityStream =
  | "BASELINE_FORMAL_STREAM"
  | "BASELINE_PRE_SCORE_ELIGIBLE_STREAM"
  | "NEW_ENTRY_EVENT_STREAM";

export type R10OpportunityIntent = Readonly<{
  candidateId: typeof M3_R10_CANDIDATE_IDS[number] | "R10-CONTROL-BASELINE-001" | "R10-PRE-SCORE";
  stream: R10OpportunityStream;
  symbol: ResearchSymbol;
...
function requireNumber(value: number | null | undefined, label: string): number {
  if (!finite(value)) throw new Error(`R10 decision-time value unavailable: ${label}.`);
  return value;
}

export function createR10FeatureContext(data: BacktestData): R10FeatureContext {
  ...
  })])) as Record<ResearchSymbol, R10SymbolIndicatorContext>;
...
function previousQuoteVolumeMean(candles: readonly Candle[], index: number): number {
  ...
  if (values.length !== 20) throw new Error("R10 volume feature lacks its closed 20-candle baseline.");
  const mean = values.reduce((sum, value) => sum + value, 0) / values.length;
  if (!finite(mean) || mean <= 0) throw new Error("R10 volume feature has an invalid closed baseline.");
  return mean;
}

/** Builds all ten fixed features from decision-time closed candles only. */
export function buildR10FeatureVector(context: R10FeatureContext, result: BacktestSignalResult): R10FeatureVector {
  const selected = context.bySymbol[result.snapshot.symbol];
  if (!selected) throw new Error(`R10 feature context is missing ${result.snapshot.symbol}.`);
  ...
  if (index1h < 20 || index4h < 5) throw new Error("R10 feature warm-up window is incomplete.");
  ...
  if (btcIndex < 12) throw new Error("R10 BTC relative-return feature lacks its closed 12-candle baseline.");
  ...
  if (!values.every(finite)) throw new Error("R10 feature vector contains a non-finite value.");
...
/** R10 uses ATR14_1h / close1h, not an average high-low proxy. */
export function r10VolatilityBucket(context: R10FeatureContext, symbol: ResearchSymbol, index: number): string {
...
export function classifyR10Router(context: R10FeatureContext, result: BacktestSignalResult): Readonly<{ routerCell: string }> {
  const selected = context.bySymbol[result.snapshot.symbol];
  ...
  const feature = buildR10FeatureVector(context, result);
  ...
  return Object.freeze({ routerCell: [
    ...
    r10VolatilityBucket(context, result.snapshot.symbol, index),
  ].join("|") });
}
...
export function r10PeriodFor(time: number): "DEV" | "OOS" {
  if (time < BACKTEST_PERIOD_RANGES.DEV.startTime || time > BACKTEST_PERIOD_RANGES.OOS.endTime) {
    throw new Error(`R10 decision time is outside the research range: ${time}.`);
  }
  return periodFor(time);
}

function eventCandidate(
  context: R10FeatureContext,
  symbol: ResearchSymbol,
  direction: StrategyDirection,
  index: number,
  eventIdentity?: R10OpportunityIntent["eventIdentity"],
  ...
  const risk = eventIdentity?.event === "E1_RECLAIM"
    ? buildR10E1RiskGeometry({ direction, entryReference: candle.close, atr14_1h: atr, previousFiveClosedCandles: selected.candles1h.slice(index - 5, index) })
    : eventIdentity?.event === "E2_RETEST" && eventIdentity.sourceBreakoutTime !== undefined
      ? (() => {
          ...
          return breakoutIndex >= 0 && breakoutIndex <= index
            ? buildR10E2RiskGeometry({ direction, entryReference: candle.close, atr14_1h: atr, breakoutThroughReclaimClosedCandles: selected.candles1h.slice(breakoutIndex, index + 1) })
            : null;
        })()
      : null;
  if (!risk) return null;
...
function interacted(context: R10SymbolIndicatorContext, index: number): boolean {
...
export function isR10E1PullbackReclaim(context: R10FeatureContext, symbol: ResearchSymbol, direction: StrategyDirection, index: number): boolean {
  const selected = context.bySymbol[symbol];
...
export function findR10E2BreakoutSource(context: R10FeatureContext, symbol: ResearchSymbol, direction: StrategyDirection, index: number): number | null {
  const selected = context.bySymbol[symbol];
  const current = selected?.candles1h[index];
  const previous = selected?.candles1h[index - 1];
...
function intent(
  context: R10FeatureContext,
  candidateId: R10OpportunityIntent["candidateId"],
  stream: R10OpportunityStream,
  symbol: ResearchSymbol,
  direction: StrategyDirection,
  index: number,
  eventIdentity?: R10OpportunityIntent["eventIdentity"],
): R10OpportunityIntent | null {
  const selected = context.bySymbol[symbol];
...
export function generateR10EventIntents(data: BacktestData, context = createR10FeatureContext(data)): readonly R10OpportunityIntent[] {
  const intents: R10OpportunityIntent[] = [];
  for (const symbol of RESEARCH_SYMBOLS) {
    ...
    for (let index = 20; index < selected.candles1h.length - 24; index += 1) {
      ...
      for (const direction of ["LONG", "SHORT"] as const) {
        if (isR10E1PullbackReclaim(context, symbol, direction, index)) {
          const value = intent(context, "R10-E1-PULLBACK-RECLAIM", "NEW_ENTRY_EVENT_STREAM", symbol, direction, index, { event: "E1_RECLAIM" });
          if (value) intents.push(value);
        }
        const breakoutTime = findR10E2BreakoutSource(context, symbol, direction, index);
        if (breakoutTime !== null) {
          const value = intent(context, "R10-E2-BREAKOUT-RETEST", "NEW_ENTRY_EVENT_STREAM", symbol, direction, index, { event: "E2_RETEST", sourceBreakoutTime: breakoutTime });
          if (value) intents.push(value);
        }
      }
    }
...
export function generateR10BaselineIntents(data: BacktestData): readonly R10OpportunityIntent[] {
    ...
    for (const evaluation of engine.evaluations) {
      ...
      if (candidate.formalSignal && candidate.totalScore >= 70) {
        intents.push(Object.freeze({
          candidateId: "R10-CONTROL-BASELINE-001",
...
export function r10FeatureNames(): readonly string[] {
  return R10_FEATURE_NAMES;
}
