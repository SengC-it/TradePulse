...
import { checkpointExists, finalPerformanceCheckpointPath, foldCheckpointPath, markR16FinalCheckpointComplete, readR16Checkpoint, recordR16PerformanceFoldCompleted, validateR16CompletedCheckpoints, validateR16ExecutionLedgerForLock, writeR16CheckpointAtomic, type R16ExecutionLock, type R16PerformanceExecutionLedger } from "./m3-r16-round-016-checkpoints.ts";
...
export async function executeR16Performance(input: Readonly<{ root?: string; executionDirectory: string; executionLock: R16ExecutionLock; executionLedger: R16PerformanceExecutionLedger; continuation: boolean; observationFreeze: R16ObservationFreezeManifest; conformance: R16ConformanceDocument }>): Promise<Readonly<{ report: R16PerformanceReport; reusedCompletedCheckpoints: number; recomputedCompletedCheckpoints: number }>> { const root = path.resolve(input.root ?? process.cwd()); validateR16Plan(); const performanceExecutionCount = validateR16ExecutionLedgerForLock(input.executionLedger, input.executionLock, root, input.executionDirectory); if (input.conformance.resultAffectingDeviationCount !== 0 || input.conformance.integrity !== "COMPLETE") throw new Error("R16 conformance is not complete."); const hashes = Object.freeze({ executionSourceSha: input.executionLock.executionSourceSha, observationDatasetSha256: input.observationFreeze.observationDataSha256, planSha256: R16_PLAN_SHA256, gateSha256: R16_GATE_SHA256, conformanceSha256: hash(input.conformance) }); validateR16CompletedCheckpoints({ root, ledger: input.executionLedger, executionDirectory: input.executionDirectory, inputHashes: hashes, continuation: input.continuation }); const finalPath = finalPerformanceCheckpointPath(input.executionDirectory); if (input.executionLedger.finalCheckpointComplete) { const storedReport = readR16Checkpoint<FinalPayload>(finalPath, hashes).payload.report; return Object.freeze({ report: Object.freeze({ ...storedReport, performanceExecutionCount }), reusedCompletedCheckpoints: R16_FOLD_IDS.length, recomputedCompletedCheckpoints: 0 }); } const observationFile = path.resolve(root, input.observationFreeze.observationDataPath); const rows = await collectRows(observationFile); const folds: FoldPerformance[] = []; const provenance: ModelProvenance[] = []; const checkpointReuse: boolean[] = []; let ledger = input.executionLedger; for (const foldId of R16_FOLD_IDS) { const result = await executeFold({ rows, foldId, executionDirectory: input.executionDirectory, inputHashes: Object.freeze({ ...hashes, foldId }) }); folds.push(result.performance); provenance.push(...result.provenance); checkpointReuse.push(result.reused); if (!ledger.completedFoldIds.includes(foldId)) ledger = recordR16PerformanceFoldCompleted({ root, expectedLedger: ledger, foldId }); } const checkpointSummary = summarizeR16CompletedCheckpoints(checkpointReuse); const report = buildReport({ executionId: input.executionLock.executionId, sourceSha: input.executionLock.executionSourceSha, freeze: input.observationFreeze, conformance: input.conformance, folds, provenance, performanceExecutionCount, continuationCount: input.executionLock.continuationCount, reusedCompletedCheckpoints: checkpointSummary.reusedCompletedCheckpoints, recomputedCompletedCheckpoints: checkpointSummary.recomputedCompletedCheckpoints }); writeR16CheckpointAtomic({ filePath: finalPath, kind: "FINAL_PERFORMANCE", key: input.executionLock.executionId, inputHashes: hashes, payload: Object.freeze({ report }) }); markR16FinalCheckpointComplete({ root, expectedLedger: ledger }); return Object.freeze({ report, ...checkpointSummary }); }
