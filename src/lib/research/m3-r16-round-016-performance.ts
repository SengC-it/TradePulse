...
import { checkpointExists, finalPerformanceCheckpointPath, foldCheckpointPath, readR16Checkpoint, writeR16CheckpointAtomic, type R16ExecutionLock } from "./m3-r16-round-016-checkpoints.ts";
...
function buildReport(input: Readonly<{ executionId: string; sourceSha: string; freeze: R16ObservationFreezeManifest; conformance: R16ConformanceDocument; folds: readonly FoldPerformance[]; provenance: readonly ModelProvenance[]; continuationCount: number; recomputedCompletedCheckpoints: number }>): R16PerformanceReport {
...
export async function executeR16Performance(input: Readonly<{ root?: string; executionDirectory: string; executionLock: R16ExecutionLock; observationFreeze: R16ObservationFreezeManifest; conformance: R16ConformanceDocument }>): Promise<Readonly<{ report: R16PerformanceReport; reusedFoldCount: number; recomputedCompletedCheckpoints: number }>> { const root = path.resolve(input.root ?? process.cwd()); validateR16Plan(); if (input.conformance.resultAffectingDeviationCount !== 0 || input.conformance.integrity !== "COMPLETE") throw new Error("R16 conformance is not complete."); const observationFile = path.resolve(root, input.observationFreeze.observationDataPath); const rows = await collectRows(observationFile); const finalPath = finalPerformanceCheckpointPath(input.executionDirectory); const hashes = Object.freeze({ executionSourceSha: input.executionLock.executionSourceSha, observationDatasetSha256: input.observationFreeze.observationDataSha256, planSha256: R16_PLAN_SHA256, gateSha256: R16_GATE_SHA256, conformanceSha256: hash(input.conformance) }); if (checkpointExists(finalPath)) return Object.freeze({ report: readR16Checkpoint<FinalPayload>(finalPath, hashes).payload.report, reusedFoldCount: R16_FOLD_IDS.length, recomputedCompletedCheckpoints: 0 }); const folds: FoldPerformance[] = []; const provenance: ModelProvenance[] = []; let reusedFoldCount = 0; for (const foldId of R16_FOLD_IDS) { const result = await executeFold({ rows, foldId, executionDirectory: input.executionDirectory, inputHashes: Object.freeze({ ...hashes, foldId }) }); folds.push(result.performance); provenance.push(...result.provenance); if (result.reused) reusedFoldCount += 1; } const report = buildReport({ executionId: input.executionLock.executionId, sourceSha: input.executionLock.executionSourceSha, freeze: input.observationFreeze, conformance: input.conformance, folds, provenance, continuationCount: input.executionLock.continuationCount, recomputedCompletedCheckpoints: reusedFoldCount }); writeR16CheckpointAtomic({ filePath: finalPath, kind: "FINAL_PERFORMANCE", key: input.executionLock.executionId, inputHashes: hashes, payload: Object.freeze({ report }) }); return Object.freeze({ report, reusedFoldCount, recomputedCompletedCheckpoints: reusedFoldCount }); }
