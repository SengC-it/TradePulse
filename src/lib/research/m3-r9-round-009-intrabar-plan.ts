...
import { M3_R9_CANDIDATE_IDS, M3_R9_CONTROL_ID, M3_R9_PERFORMANCE_LOCK, M3_R9_RESEARCH_ROUND_ID } from "./m3-r9-round-009-protocol.ts";
import { R9_PLAN_SHA256 } from "./m3-r9-round-009-plan.ts";
import { stableStringify } from "./utils.ts";
import type { R9OpportunityIntent } from "./m3-r9-round-009-candidates.ts";

export const M3_R9_INTRABAR_PLAN_VERSION = "m3-r9-round-009-intrabar-plan-001" as const;

export type R9IntrabarPlan = Readonly<{
  planVersion: typeof M3_R9_INTRABAR_PLAN_VERSION;
  researchRoundId: typeof M3_R9_RESEARCH_ROUND_ID;
  sourceSha: string;
  candidatePlanSha256: typeof R9_PLAN_SHA256;
...
export function discoverR9IntrabarRequirements(input: Readonly<{ data: BacktestData; intents: readonly R9OpportunityIntent[] }>): readonly IntrabarSettlementRequirement[] {
  ...
  for (const value of values) {
    ...
    if (prior && stableStringify(prior) !== stableStringify(value)) throw new Error(`Conflicting R9 intrabar requirement: ${identity}`);
...
export function buildR9IntrabarPlan(input: Readonly<{ data: BacktestData; intents: readonly R9OpportunityIntent[]; sourceSha: string }>): R9IntrabarPlan {
  const raw = discoverR9IntrabarRequirements({ data: input.data, intents: input.intents });
  ...
  for (const requirement of raw) {
    ...
    if (existing && stableStringify(existing) !== stableStringify(requirement)) throw new Error(`Conflicting R9 intrabar requirement: ${identity}`);
  ...
  const declarationHash = sha256({ planVersion: M3_R9_INTRABAR_PLAN_VERSION, researchRoundId: M3_R9_RESEARCH_ROUND_ID, sourceSha: input.sourceSha, candidatePlanSha256: R9_PLAN_SHA256, declarations: declarations.map((declaration) => declaration.identity) });
  return Object.freeze({
    planVersion: M3_R9_INTRABAR_PLAN_VERSION,
    researchRoundId: M3_R9_RESEARCH_ROUND_ID,
    sourceSha: input.sourceSha,
    candidatePlanSha256: R9_PLAN_SHA256,
    consumers: Object.freeze([M3_R9_CONTROL_ID, ...M3_R9_CANDIDATE_IDS]),
...
export function persistR9IntrabarPlan(plan: R9IntrabarPlan, filePath: string): void {
...
export function r9DatasetIdentity(input: Readonly<{ data: BacktestData; plan: R9IntrabarPlan; studyServerTime: number }>): string {
  return sha256({
...
export function readR9IntrabarPlan(filePath: string): R9IntrabarPlan {
  return JSON.parse(readFileSync(path.resolve(filePath), "utf8")) as R9IntrabarPlan;
}
