...
export default async function DashboardPage() {
  ...
  const reviewPerformanceNote = overview.reviewMetrics.hasValidSample ? "基于已结算正式信号的理论 R 表现" : "暂无已结算 TP / SL 复盘样本";
  return (
    <>
      ...
      <section className="dashboard-section" aria-labelledby="today-title">
        ...
        <div className="metric-grid">
          ...
          <MetricCard label="未完成复盘" value={String(overview.pendingReviews)} detail="待首次复盘、待入场或观察中的正式信号" tone={overview.pendingReviews > 0 ? "warning" : "neutral"} />
        </div>
      </section>

      <section className="dashboard-section" aria-labelledby="performance-title">
        <div className="section-heading"><div><p className="eyebrow">只读统计</p><h2 id="performance-title">策略表现</h2></div><span className="section-note">{reviewPerformanceNote}</span></div>
